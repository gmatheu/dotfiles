...
print_bytes() {
    ...
    # else
    bytes="$(echo "$1/1000000" | bc -l | LANG=C xargs printf "%.2f\n")"
    # fi
...
INTERVAL=10
INTERFACES="$(nmcli device status | grep -e wifi | grep -v 'disconnected' | cut -d ' ' -f 1)"
...
while true; do
    down=0
    up=0
    signal=$(nmcli -f IN-USE,SIGNAL dev wifi | grep '*' | tr -s ' ' | cut -d ' ' -f 2)
    ...
    echo "↓$(print_bytes $down) ↑$(print_bytes $up) ${signal}%"
